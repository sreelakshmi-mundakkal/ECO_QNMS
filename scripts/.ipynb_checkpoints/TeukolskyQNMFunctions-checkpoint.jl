...
export F, compute_om, F_dS, compute_omdS

include("Spheroidal.jl")
include("RadialODE.jl")

#include("AngularODEdS.jl")
include("RadialODE_ECO.jl")

import .Spheroidal as SH
import .RadialODE as RD
import .RadialODE_ECO as DRD
...
Compute the absolute difference of Lambda seperation constant for radial and angular ODEs for deSitter Case.
"""
function F_dS(
    nr::Integer,
    nl::Integer,
    ...
    la_s, _= SH.eig_vals_vecs(nl, neig, s, m, a * om)
    la_r, _, _ = DRD.eig_vals_vecs(nr, s, m, a,eps, om)
...
function compute_omdS(
    ...
    f(omega) = F_dS(nr, nl, s, l, m, a, eps, omega)
    ...
    la_s, v_s = SH.eig_vals_vecs(nl, neig, s, m, a * om_np1)
    la_r, v_r, rvals = DRD.eig_vals_vecs(nr, s, m, a,eps, om_np1)
